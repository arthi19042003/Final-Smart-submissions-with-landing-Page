...
router.post(
  ...
  [
    ...
    body("role").notEmpty().isIn(["candidate", "employer", "hiringManager", "recruiter", "admin"]).withMessage("Invalid role"),
  ],
  async (req, res) => {
    try {
      ...
      // LOGIC CHANGE: Only Candidates and Admins are auto-approved
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // LOGIC CHANGE: Check if user is approved
      if (!user.isApproved) {
